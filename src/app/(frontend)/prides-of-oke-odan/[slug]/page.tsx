...
export async function generateMetadata({ params: paramsPromise }: Args): Promise<Metadata> {
  ...
  return generateMeta({ doc: post, url: '/prides-of-oke-odan/' + slug })
}
...
const PridePage = async ({ params: paramsPromise }: Args) => {
  const { slug } = await paramsPromise
  const payload = await getPayload({ config: configPromise })
  ...
  if (typeof featuredImage === 'string') return <></>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
        ...
        <aside className="lg:col-span-1">
          <div className="bg-white rounded-lg shadow-sm p-6 sticky top-4">
            <h2 className="text-xl font-semibold text-gray-900 mb-6 pb-2 border-b">Other People</h2>

            <div className="space-y-6">
              {others.map((article, index) => (
                <div key={index} className="group cursor-pointer">
                  <div className="w-full h-40 relative rounded-lg mb-3">
                    <Image
                      src={
